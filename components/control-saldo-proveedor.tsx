...
export default function ControlSaldoProveedor() {
    ...
    const fetchProveedores = async () => {
        ...
        setProveedores(data || [])
        if (!selectedProv && data && data.length > 0) setSelectedProv(data[0].id)
    }
    ...
    const handleCargarSaldo = async () => {
        ...
        try {
            ...
            // 1. Intentar actualizar saldo vía RPC (Función de base de datos)
            const { error: rpcError } = await supabase.rpc('incrementar_saldo_proveedor', { 
               id_input: selectedProv,
               monto_input: monto
            })

            // 2. Fallback: Si la función RPC no existe o falla, usar UPDATE normal
            // Esto evita el error de TypeScript con .catch()
            if (rpcError) {
                console.warn("RPC falló, usando método manual:", rpcError.message)

                // A. Obtenemos saldo actual
                ...
                // B. Calculamos nuevo saldo
                const nuevoSaldo = (actual?.saldo_actual || 0) + monto
